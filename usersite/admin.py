...
from .models import GeneralDayDescriptionModel, DayEventModel, DayPhotoModel, DayVideoModel, DayAudioModel, \
...
class DayPhotoAdmin(admin.ModelAdmin):
    list_display = ['id', 'parent', 'parent_event_time', 'description', 'media', 'category']
...
class DayVideoAdmin(admin.ModelAdmin):
    list_display = ['id', 'parent', 'parent_event_time', 'category', 'description', 'media']
    list_display_links = ['id', 'parent']
    search_fields = ['parent']
    ordering = ['parent']
    list_per_page = 25
...
class DayArticleAdmin(admin.ModelAdmin):
    list_display = ['id', 'parent', 'parent_event_time', 'category', 'description']
    list_display_links = ['id', 'parent']
    search_fields = ['parent']
    ordering = ['parent']
    list_per_page = 25
...
class LostItemsListAdmin(admin.ModelAdmin):
    list_display = ['team', 'tanks', 'battle_armored_vehicle', 'artilery_system',
                    'rszo', 'antyair_missle', 'airplanes', 'helicopters', 'vehicles',
                    'ships_and_boats', 'bpla', 'special_vehicle', 'rockets']
