...
from usersite.models import GeneralDayDescriptionModel, LostItemsListModel, DayEventModel, DayPhotoModel, DayVideoModel, DayAudioModel, DayArticleModel
...
def index(request):
    ...
    return render(request, 'usersite/index.html', context=context)
...
def day_review(request, pk=None):
    ...
    return render(request, 'usersite/day_review.html', context=context)


def photo(request):
    photos = DayPhotoModel.objects.all()
    return render(request, 'usersite/photos.html', {'photos': photos})


def video(request):
    videos = DayVideoModel.objects.all()
    return render(request, 'usersite/videos.html', {'videos': videos})


def audio(request):
    audios = DayAudioModel.objects.all()
    return render(request, 'usersite/audios.html', {'audios': audios})
...
def current_event(request, pk=None):
    ...
    return render(request, 'usersite/current_event.html', context=context)


def articles(request, pk=None):
    articles = DayArticleModel.objects.all()
    return render(request, 'usersite/current_event.html', {'articles': articles})
...
def contacts(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            mail = send_mail(form.cleaned_data['subject'], form.cleaned_data['message'], 'eugene_chernyshev@ukr.net', ['eugene_chernyshev@ukr.net'], fail_silently=False)
            if mail:
        ...
        form = ContactForm()
    return render(request, 'usersite/contact.html', {'form': form})
...
def day_list(request):
    ...
    return render(request, 'usersite/day_list.html', {'day_list': day_list, 'day_list_object': day_list_object})
